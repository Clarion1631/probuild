...
async function main() {
    try { await runTimeEntryApply(applyClockInIntegrity); console.log("Clock-in integrity schema applied."); }
    catch { console.error("Clock-in integrity refused or failed. Check --target, APPLY_EXPECT_PROJECT_REF, target file and migration compatibility; no automatic data repair."); process.exitCode = 1; }
}
