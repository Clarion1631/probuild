...
// Speed-to-Lead v1a (PB-leads-001). Additive DDL only — see
// prisma/migrations/20260926120000_speed_to_lead_v1a/migration.sql, which
// this script executes verbatim. Run manually against a target before
// deploying; see the probuild-schema-migration and deploy-probuild skills.
//
...
//
// Deliberately NOT run against production by this PR (hard rule: never
// apply migrations to prod) — this file exists so Justin can run it later
// (R1), per the deploy-probuild skill.
...
async function verifyShape(tx) {
  ...
  if (leadCols.length !== NEW_LEAD_COLUMNS.length) throw new Error('Lead tracking columns are missing');
...
async function main() {
  ...
  try {
    ...
    const sql = readFileSync(new URL('../prisma/migrations/20260926120000_speed_to_lead_v1a/migration.sql', import.meta.url), 'utf8');
    ...
    // marker inside a sentence, and a plain sql.split('-- statement-break')
    // would cut that quoted text in half, producing an invalid fragment.
    ...
    console.log('Speed-to-Lead v1a schema applied.');
  } finally {
...
if (isMainModule) {
  main().catch((error) => {
    console.error('Speed-to-Lead v1a migration refused or failed:', error instanceof Error ? error.message : error);
    process.exitCode = 1;
  });
}
