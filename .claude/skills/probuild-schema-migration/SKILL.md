...
Every apply script must be **inert on import**: dotenv loading, `new PrismaClient(...)`, and every DDL/query execution live inside `async function main()` or inside the guard block (SQL string constants and helper function definitions may stay at module scope). The guard is `const isMainModule = process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href;` followed by exactly one `if (isMainModule) { ... }` block, and the test rejects an inline condition. New scripts end that block with `await main();`. The older chain shape is also accepted: preflight statements that never mention `main` (flag checks, `prisma = new PrismaClient(...)` into a module-scope `let`), then a final `main().catch(...).finally(() => prisma.$disconnect())`. Either way the block must end with exactly one unconditional call to `main()`. `tests/apply-scripts-inert-on-import.test.ts` (in `npm run test:unit`) checks that exact guard shape, allows only imports with bindings from an exact module allowlist, function declarations, inert `const`/`let` declarations, and plain `export { ... }` lists at module scope (so no dotenv, PrismaClient, raw SQL, unguarded top-level `await`, `process.exit`, re-export, or any other call runs on import), and imports every `scripts/apply-*.mjs` in a scrubbed child process with `DATABASE_URL` pointed at a dummy TCP listener that must see no connection. It detects that specific syntax plus observed connections, not arbitrary side effects. Run a script by its real path: through a symlink or junction the guard no-ops silently (you will see no `applied` output). Reason: on 2026-09-02 an `import` of `apply-payroll-phase5.mjs`, done only to look at its exports, ran the whole migration against production. Never import an apply script to inspect it — read it as text.
