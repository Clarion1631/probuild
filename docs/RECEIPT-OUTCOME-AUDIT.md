...
The audit clock is captured after the consistent read completes, so records posted during collection are not mistakenly judged future-dated. The review count is distinct targets with contradiction flags (identity or artifact conflicts, mismatched or unbacked memo evidence, malformed issue details, or unverified/unknown card status), plus distinct collection-error codes not already represented by those target flags. The explicit error-to-flag mapping prevents the same finding counting twice. Ordinary no-card/missing-issue, incomplete-source, and elapsed-time flags alone do not add a review item. This count is review items, not a count of every underlying bad record; a collection error without a target is counted once by error kind.
