/**
 * PROBUILD EXPENSE STEP — add as a new file "sendToProBuild.gs" in the
 * "QBO Automation" Apps Script project, then add ONE line to
 * runReceiptAutomation.gs inside processSingleFile(), between step 5
 * (EMAIL TO QUICKBOOKS) and step 6 (HOUZZ TXT FILES):
 *
 *     // 5.5 PROBUILD EXPENSES (project docs only; Shop/overhead stays QBO-only)
 *     sendExpensesToProBuild(file, ctx, aiData, isCheck, categoryGroups, totalAmount, dateStr, cleanInv, memo, state);
 *
