...
**Live auto-apply is a switch, and it is OFF.** Whether an amount-only match may move real money is Decision 2 in the plan, not a default, so it is gated on the environment variable `DEPOSIT_SWEEP_LIVE_APPLY` being exactly the string `true`. Without it the sweep is suggest-only: a credit that passes every rule is recorded as `proposed` for a human to confirm. A credit that carries payer corroboration (a check image naming the customer) is not gated by the switch, and a wrong-payer image is a conflict either way.
