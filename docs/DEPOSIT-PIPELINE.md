...
| **Serialized claim domain**: both reservation paths take `pg_advisory_xact_lock` on the amount before the claim check | The check is a read-then-write, and under READ COMMITTED two transactions at the same amount could both read "nobody else is claiming this" and both write. The lock makes the second one wait and then see the first one's row. |
...
| **Chronology**: the milestone must already have been requested when the money arrived (`qbInvoiceSentAt` at or before the end of the credit's post date, company time) | Money cannot pay a bill that had not been sent yet; without it, invoicing a new milestone at the same amount today would retroactively make it a candidate for last week's deposit. `qbInvoiceSentAt` is a LAST-send timestamp, so a re-send after the credit pushes the row out of auto-apply and into human review — the safe direction. |
...
| **A credit with no Bank Reference is excluded, not fatal** | The reference is the idempotency key, so such a credit cannot be swept at all — but refusing the day for it also stopped every LATER day, so one row on 2026-08-28 would have failed the 6pm job forever. The runner declares it instead: the batch carries an `excluded` list and the bank's unchanged day totals, and the endpoint accepts it only when credits + excluded tie to both. Nothing is recorded for an excluded row; if it looks like a customer deposit the job logs a WARNING naming the amount and date, which is not a failure. |
| **Replay identity**: a reference is only a replay of itself | `bank:<reference>` is the idempotency key, so the row also stores a fingerprint of the credit (post date, amount, BAI code, description, detail). The same reference carrying different money is `reconcile`, never a silent overwrite of the original deposit. |
...
2. **Job progress** — the field independently says *this phase* is done, within 21 days before the deposit. Two rungs of evidence, and both must be about the milestone in hand:
   ...
   Matching uses the milestone's distinctive words (name minus stop words like *complete*, *payment*, *final*, tokens of 4+ characters), so a milestone called "Final Payment" has nothing to match on and can never be corroborated this way. **All** of those words must appear in the same inspection type or the same log entry (a "Rough-in plumbing" sign-off does not corroborate "Electrical Rough Complete"), and a log sentence that names the phase alongside a negation — *not, no, pending, waiting, delayed, still, needs, tomorrow, next week* — is read as describing what is **left**, so "cabinets not delivered" never corroborates a Cabinets milestone. Justin's rule, 2026-09-02: the money should align with the daily logs from the chat spaces. Books on its own.
...
| Sweep trigger (Hermes daily job) | `scripts/parse-wtb-daily-csv.mjs --sweep` / `--sweep-dry-run` (unknown flags are refused: a typo must never post a live batch believing it is shadowing) |
| Live-apply switch | `DEPOSIT_SWEEP_LIVE_APPLY=true` in the app's environment — rung 3 of the ladder, off by default |
| Sweep secret | `DEPOSIT_INGEST_SECRET` in the job's environment (same value as the bot's) |
| State machine table | `DepositIngest` (schema: `scripts/apply-deposit-ingest-schema.mjs`, then `scripts/apply-deposit-sweep-schema.mjs`, then `scripts/apply-deposit-sweep-fingerprint.mjs`) |
