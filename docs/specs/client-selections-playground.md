...
- **The client has full control of their own space** (Justin, 2026-07-28, reaffirmed after the "protect the approved record" concern was raised): they choose, un-choose, archive, and DELETE anything on their project — including decisions the team created and decisions already decided. No createdByClient gate, no status gate. Deletion is implemented as a **soft delete** (`Decision.deletedAt`) so their view behaves as a true delete while nothing is actually destroyed; the team gets a 30-day restore. This satisfies "it's their playground" and "lose no customer data" at once.
