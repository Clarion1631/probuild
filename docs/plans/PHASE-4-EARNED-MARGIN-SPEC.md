...
5. **Canceled payment schedules (raised in Codex review of this phase; NOT introduced by it).** `computeProjectFinancials` skips `payment.status === "Canceled"` when accumulating `scheduledIncoming`/`overdueIncoming`, which flows into `clientOwes`, `totalForecastedIncoming` and `forecastedMargin`. Verified on `origin/main` at `src/lib/project-financials.ts:87`, with its own explanatory comment, and the Phase 4 diff deletes no line in that loop — so it is pre-existing behaviour, not an additive-only violation here. Left exactly as it is. If that rule is wrong it is a separate change with its own before/after numbers, and reverting it inside this PR would itself be the regression.
6. **OPEN (one constant, decide any time):** the 0.5 progress convention for a task-less phase with a daily-log mention is coarse. Executor ships 0.5 in `percent-complete.ts` unless Justin picks a different convention; it is a single named constant with a test.
