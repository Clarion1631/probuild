                                ...
                                {ccEmails.map(email => {
                                    ...
                                    return (
                                        <span key={email} className="bg-slate-100 text-slate-700 text-xs px-2 py-0.5 rounded-full flex items-center gap-1 border border-slate-200 shadow-sm">
                                            {tm ? tm.name : email}
                                            <button type="button" onClick={(e) => { e.stopPropagation(); setCcEmails(prev => prev.filter(m => m !== email)); }} className="hover:text-red-500 ml-0.5" aria-label="Remove email">×</button>
                                        </span>
                                    );
                                })}
                                <input
