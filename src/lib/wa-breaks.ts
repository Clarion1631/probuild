...
export const MEAL_WORKED_NOTE = "Worker reported a worked-through or interrupted meal at clock-out";
export const MEAL_DERIVED_NOTE = "Meal outcome derived from another punch, no meal answer captured on this punch";
...
export function applyNoAttestationNotice(input: {
    ...
    const answered = typeof input.mealSkipped === "boolean" && !["DEFERRED", "NOT_REQUIRED"].includes(input.outcome);
    const parts = reviewReasonParts(input.existingReviewReason).filter(part => !answered || ![MEAL_DERIVED_NOTE, MEAL_CONFIRMED_NOTE, MEAL_WORKED_NOTE, NO_ATTESTATION_NOTE].includes(part));
    if (answered && input.mealSkipped === false) parts.push(MEAL_CONFIRMED_NOTE);
    if (answered && input.mealSkipped === true) parts.push(MEAL_WORKED_NOTE);
    if (input.outcome === "MEAL_REVIEW") {
        return { needsReview: true, reviewReason: [...new Set([...parts, MEAL_REVIEW_NOTE])].join("; ") };
    }
    if (answered) return { reviewReason: [...new Set(parts)].join("; ") };
 ...
 */
export function recordedMealAnswer(entry: { mealOutcome?: string | null; reviewReason?: string | null }): boolean | undefined {
    const parts = reviewReasonParts(entry.reviewReason);
    if (parts.includes(MEAL_DERIVED_NOTE)) return undefined;
    if (parts.includes(MEAL_WORKED_NOTE) || entry.mealOutcome === "WORKED_THROUGH") return true;
    if (parts.includes(MEAL_CONFIRMED_NOTE)) return false;
    // Legacy rows predate explicit evidence notes. Never infer an answer from a
    // newly derived day outcome or a legacy missing-answer notice.
    if (entry.mealOutcome === "AUTO_DEDUCTED" && !parts.includes(NO_ATTESTATION_NOTE)) return false;
    return undefined;
}
...
function dayAttestation(entries: SettleDayEntry[]): { workedThrough: boolean; answered: boolean; approved: boolean } {
    ...
    for (const entry of entries) {
        ...
        const answer = recordedMealAnswer(entry);
        if (answer === true) {
            workedThrough = true;
            answered = true;
...
export function settleDayPlan(input: {
    ...
    return entries.map((entry): SettleDayUpdate => {
        ...
        const freshAnswer = input.closing?.id === entry.id && typeof input.closing.mealSkipped === "boolean";
        const ownAnswer = freshAnswer ? input.closing!.mealSkipped : recordedMealAnswer(entry);
        if (ownAnswer === false) wanted.push(MEAL_CONFIRMED_NOTE);
        if (ownAnswer === true) wanted.push(MEAL_WORKED_NOTE);
        if (ownAnswer === undefined && isLast && ["AUTO_DEDUCTED", "WORKED_THROUGH"].includes(dayOutcome)) wanted.push(MEAL_DERIVED_NOTE);
        if (overlapping.has(entry.id)) wanted.push(OVERLAP_NOTE);
        const parts = reviewReasonParts(entry.reviewReason);
        const others = parts.filter((part) => !SETTLEMENT_NOTES.includes(part) && (!freshAnswer || ![MEAL_DERIVED_NOTE, MEAL_CONFIRMED_NOTE, MEAL_WORKED_NOTE].includes(part)));
        const desired = [...new Set([...others, ...wanted])];
        const unchanged = desired.length === parts.length && desired.every((part, i) => part === parts[i]);
        if (wanted.some((note) => ![MEAL_CONFIRMED_NOTE, MEAL_WORKED_NOTE, MEAL_DERIVED_NOTE].includes(note))) {
            ...
            if (desired.every(note => [MEAL_CONFIRMED_NOTE, MEAL_WORKED_NOTE, MEAL_DERIVED_NOTE].includes(note))) update.needsReview = false;
        }
        return update;
    });
}
