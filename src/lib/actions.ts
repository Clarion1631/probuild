...
export async function approveEstimate(
  ...
  const clientName =
    estimate?.project?.client?.name || estimate?.lead?.client?.name || signatureName;
...
export async function saveEstimate(
  ...
  await prisma.estimate.update({
    ...
    data: {
      ...
      ...(data.processingFeePercent !== undefined && {
        processingFeePercent: data.processingFeePercent
          ? parseFloat(data.processingFeePercent)
          : null,
      }),
...
export async function saveCompanySettings(data: any) {
  await prisma.companySettings.update({
    ...
    data: {
      ...
      cardProcessingRate:
        data.cardProcessingRate !== undefined ? parseFloat(data.cardProcessingRate) : undefined,
      cardProcessingFlat:
        data.cardProcessingFlat !== undefined ? parseFloat(data.cardProcessingFlat) : undefined,
    },
  });
...
export async function updateDocumentTemplate(
  id: string,
  data: { name?: string; type?: string; body?: string; isDefault?: boolean }
) {
  if (data.isDefault) {
...
async function buildMergeData(
  ...
  if (projectId) {
    ...
    if (project) {
      ...
      data.estimate_total = project.estimates[0]
        ? `$${project.estimates[0].totalAmount.toLocaleString()}`
        : "$0.00";
    }
  } else if (leadId) {
    const lead = await prisma.lead.findUnique({
    ...
    if (lead) {
      ...
      data.estimate_total = lead.estimates[0]
        ? `$${lead.estimates[0].totalAmount.toLocaleString()}`
        : "$0.00";
    }
  }
...
export async function updateTaskStatusAsSub(
  taskId: string,
...
export async function assignSubToTask(taskId: string, subcontractorId: string) {
  const assignment = await prisma.subTaskAssignment.create({
    ...
    include: {
      subcontractor: { select: { id: true, companyName: true, email: true, trade: true } },
    },
  });
  const task = await prisma.scheduleTask.findUnique({ where: { id: taskId } });
  if (task) revalidatePath(`/projects/${task.projectId}/schedule`);
...
export async function clearAllTasks(projectId: string) {
  ...
  const taskIds = (
    await prisma.scheduleTask.findMany({ where: { projectId }, select: { id: true } })
  ).map((t) => t.id);
...
export async function savePortalVisibility(
  projectId: string,
  data: {
...
export async function getPurchaseOrder(id: string) {
  ...
  return prisma.purchaseOrder.findUnique({
    ...
    include: {
      ...
      files: true,
      expenses: { include: { costCode: true } },
    },
  });
}
...
export async function createPurchaseOrderFromEstimate(
  ...
  const totalAmount = selectedItems.reduce(
    (acc: number, item: any) =>
      acc + (parseFloat(item.quantity) || 0) * (parseFloat(item.unitCost) || 0),
    0
  );
...
export async function deletePurchaseOrderFile(fileId: string) {
  "use server";
  const file = await prisma.purchaseOrderFile.findUnique({
    where: { id: fileId },
  ...
  revalidatePath(
    `/projects/${file.purchaseOrder.projectId}/purchase-orders/${file.purchaseOrderId}`
  );
}
...
export async function sendPurchaseOrder(id: string, toEmail: string, message: string) {
  ...
  await sendNotification(toEmail, `Purchase Order ${po.code} - ${po.project.name}`, htmlContent, [
...
export async function deleteSelectionCategory(id: string) {
  const cat = await prisma.selectionCategory.findUnique({
    where: { id },
    include: { board: true },
  });
...
export async function submitClientSelections(boardId: string, selections: Record<string, string>) {
  ...
  const settings = await getCompanySettings();
  if (settings.notificationEmail) {
    const selectedSummary = board.categories
      .map((cat) => {
...
export async function updateDailyLog(
  ...
  if (data.materialsDelivered !== undefined)
    updateData.materialsDelivered = data.materialsDelivered || null;
...
export async function addDailyLogPhotos(
  dailyLogId: string,
...
export async function saveMoodBoardItems(
  boardId: string,
  items: Array<{
    id?: string;
    type: string;
...
export async function createLeadScheduleTask(
  leadId: string,
  data: {
    name: string;
    startDate: Date;
...
export async function updateLeadScheduleTask(
  taskId: string,
  leadId: string,
  data: {
    name?: string;
...
export async function addBidScope(
  packageId: string,
  projectId: string,
  data: {
    name: string;
...
export async function inviteSubToBid(
  packageId: string,
  projectId: string,
  data: {
    email: string;
...
// ─── Estimate File Attachments (Supabase Storage — no DB model needed) ───
...
export async function listEstimateFiles(estimateId: string): Promise<EstimateFileInfo[]> {
  "use server";
  const { getSupabase, STORAGE_BUCKET } = await import("./supabase");
  const supabase = getSupabase();
  if (!supabase) return [];
  ...
  return data
    ...
    .map((f) => {
      const storagePath = `estimates/${estimateId}/${f.name}`;
      const { data: urlData } = supabase.storage.from(STORAGE_BUCKET).getPublicUrl(storagePath);
      return {
...
export async function deleteEstimateFile(estimateId: string, storagePath: string) {
  "use server";
  if (!storagePath.startsWith(`estimates/${estimateId}/`)) {
