...
export async function readClockInReplay(db: Pick<Prisma.TransactionClient, "clockInRequest">, userId: string, requestId: string) {
    ...
    return request ? { requestHash: request.requestHash, entry: request.timeEntry?.userId === userId && !request.timeEntry.voidedAt ? request.timeEntry : null } : null;
}
...
export function clockInStore(tx: PayrollTxClient, userId: string, startTime: Date, timeZone: string, create: () => Promise<TimeEntry>): ClockInStore<TimeEntry> {
    ...
    return {
        ...
        open: () => db.timeEntry.findFirst({ where: nonVoidedTimeEntryWhere({ userId, endTime: null, durationHours: null }), orderBy: { startTime: "asc" } }),
