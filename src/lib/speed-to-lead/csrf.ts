...
export function mintOutreachCsrfToken(sessionEmail: string, messageId: string, versionId: string): string {
    return createHmac("sha256", secret()).update(`${sessionEmail}:${messageId}:${versionId}`).digest("hex");
}

export function verifyOutreachCsrfToken(token: string, sessionEmail: string, messageId: string, versionId: string): boolean {
    if (!secret()) return false;
    const expected = mintOutreachCsrfToken(sessionEmail, messageId, versionId);
