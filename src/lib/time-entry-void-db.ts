...
export async function voidTimeEntry(db: PrismaClient, input: VoidInput) {
    ...
    return db.$transaction(async tx => {
        ...
        await acquirePayrollLocks(payroll, { entryIds: ids, dayKeys: [dayLockKey(observed.userId, day)] });
        ...
        // Keep the three-day superset locked to preserve global lock ordering,
        // but use settlement's exact company-day predicate for affected rows.
        // The target is included even when open; other open/voided rows cannot
        // be rewritten by settleDayWithinTx.
        const candidates = await tx.timeEntry.findMany({ where: { id: { in: ids }, userId: row.userId, voidedAt: null, endTime: { not: null } } });
        const affected = [row, ...candidates.filter(entry => entry.id !== row.id && dayKeyInTimeZone(entry.startTime, input.timeZone) === day)];
        const affectedIds = affected.map(entry => entry.id);
        await assertPeriodUnlockedInTx(payroll, affected.map(entry => entry.startTime), { timeZone: input.timeZone });
        ...
        const snapshots = await tx.$queryRaw<Array<{ row: Prisma.JsonObject }>>`SELECT to_jsonb(t) AS row FROM "TimeEntry" t WHERE id = ANY(${affectedIds}::text[]) ORDER BY id`;
        ...
        const after = await tx.$queryRaw<Array<{ row: Prisma.JsonObject }>>`SELECT to_jsonb(t) AS row FROM "TimeEntry" t WHERE id = ANY(${affectedIds}::text[]) ORDER BY id`;
