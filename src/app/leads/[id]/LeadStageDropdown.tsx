...
export default function LeadStageDropdown({ leadId, currentStage, variant = "default" }: { leadId: string, currentStage: string, variant?: "default" | "pill" }) {
    ...
    // Canonical lead pipeline — must match the tabs on /leads, lead scoring, and
    // the "Estimate Sent" automation in actions.ts. A lead sits in this pipeline
    // until it converts to a project ("Won") or dies ("Closed Lost").
    const stages = [
        "New",
        "Followed Up",
        "Connected",
    ...
    const getStageColor = (stage: string) => {
        switch (stage) {
            ...
            case 'Connected': return 'bg-indigo-50 text-indigo-700 border-indigo-200';
