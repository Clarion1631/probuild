...
import { CopyToProjectModal } from "@/components/CopyToProjectButton";
...
export default function LeadEstimatesTable({
    leadId,
    estimates,
    allProjects,
}: {
                    ...
                    <BulkActionBar
                        ...
                        actions={[
                            ...
                            },
                            {
                                ...
                                disabled: isBulking || allProjects.length === 0,
                            },
                            {
                                label: "Delete",
                                icon: DeleteIcon,
                ...
                <table className="w-full text-sm text-left">
                    ...
                    <tbody className="divide-y divide-slate-100">
                        ...
                        {estimates.map((est) => {
                            ...
                            return (
                                <tr key={est.id} className={`hover:bg-slate-50/80 transition group ${isSelected ? "bg-indigo-50/30" : ""}`}>
                                    ...
                                    <td className="px-6 py-4">
                                        <Link href={`/leads/${leadId}/estimates/${est.id}`} className="font-medium text-hui-textMain hover:text-hui-primary transition-colors flex items-center gap-2">
                                            ...
                                            <div>
                                                {est.code && <span className="text-xs text-slate-400 font-mono">{est.code}</span>}
                                                <span className={est.code ? "ml-2" : ""}>{est.title}</span>
                                            </div>
                                        </Link>
                                    </td>
                                    ...
                                    <td className="px-4 py-4">
                                        <div className="flex items-center gap-1 justify-end">
                                            ...
                                            <DeleteEstimateButton
                                                ...
                                                status={est.status}
                                            />
                                            <Link href={`/leads/${leadId}/estimates/${est.id}`} className="text-slate-300 hover:text-slate-600 opacity-0 group-hover:opacity-100 transition">
