...
export default async function PortalSchedulePage(props: { params: Promise<{ id: string }> }) {
    ...
    // Detect if a subcontractor is viewing — subs can access regardless of portal visibility
    ...
    const tasks = rawTasks.map((t: any) => ({
        ...
        comments: (t.comments || []).map((c: any) => ({
            ...
            authorName: c.subcontractorName || c.user?.name || c.user?.email || "Unknown",
        })),
    }));
    ...
    return (
        <div className="max-w-[1400px] mx-auto py-8 px-4">
            <div className="mb-6 flex items-center justify-between">
                <Link href={subcontractorId ? `/sub-portal/projects/${id}` : `/portal/projects/${id}`} className="inline-flex items-center gap-1.5 px-3 py-1.5 text-sm font-medium text-hui-textMain bg-white border border-slate-200 rounded-md hover:bg-slate-50 transition shadow-sm w-fit">
                ...
                <div className="text-right">
                    ...
                    {subcontractorId && (
                        <p className="text-xs text-amber-600 font-medium mt-0.5">Showing your assigned tasks only</p>
                    )}
                </div>
            </div>
