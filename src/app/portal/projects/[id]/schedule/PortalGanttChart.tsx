...
export default function PortalGanttChart({ initialTasks }: { initialTasks: Task[] }) {
        ...
        return (
            <div className="flex-1 flex flex-col items-center justify-center bg-slate-50 gap-4 py-20 px-6 text-center">
                ...
                <div>
                    ...
                    <p className="text-sm text-slate-500 mt-1 max-w-sm">The project timeline has not been published yet. Please check back later.</p>
                </div>
            </div>
        );
    }
            ...
            <div className="flex flex-1 overflow-hidden">
                ...
                <div ref={scrollRef} className="flex-1 overflow-auto bg-slate-50/50 relative">
                    <div style={{ width: timelineWidth, minHeight: "100%" }} className="relative bg-white">
                        ...
                        <svg className="absolute top-0 left-0 w-full h-full pointer-events-none z-[4]" style={{ width: timelineWidth, height: 36 + tasks.length * ROW_HEIGHT }}>
                            ...
                            {arrows.map((arrow, i) => {
                                ...
                                return (
                                    <path key={`a-${i}`} d={`M ${x1} ${y1} L ${mx} ${y1} L ${mx} ${y2} L ${x2} ${y2}`} fill="none" stroke="#cbd5e1" strokeWidth="1.5" markerEnd="url(#arrowhead)" />
                                );
                            })}
                        </svg>
                                ...
                                <div key={task.id} className="absolute flex items-center px-1" style={{ top: topY, left: bar.left, width: bar.width, height: ROW_HEIGHT }}>
                                    <div className="w-full h-6 rounded-md shadow-sm relative overflow-hidden group border border-black/5" style={{ backgroundColor: task.color + "22" }}>
                                        ...
                                        <div className="relative z-[2] flex items-center justify-between h-full px-2" title={`${task.name} (${task.progress}%)`}>
                                            <span className="text-[10px] font-bold truncate leading-none transition-colors" style={{ color: task.progress > 50 ? "#fff" : task.color, textShadow: task.progress > 50 ? '0 1px 1px rgba(0,0,0,0.1)' : 'none' }}>
