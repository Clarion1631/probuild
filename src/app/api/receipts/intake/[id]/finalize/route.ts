...
async function reconcileLateFields(
    ...
    const current = await prisma.receiptIntake.findUnique({
        where: { id },
        select: { costCodeId: true, projectId: true },
    });
    ...
    if (Object.keys(toApply).length > 0) {
        // Conditional on still being null, so a concurrent writer that set it
        // between the read and here wins rather than being overwritten.
        await prisma.receiptIntake.updateMany({
            where: { id, ...Object.fromEntries(Object.keys(toApply).map(key => [key, null])) },
            data: toApply,
        });
    }
    return null;
