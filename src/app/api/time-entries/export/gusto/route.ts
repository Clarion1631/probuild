 ...
 *
 * Refuses with 409 while any entry inside the period is still open or flagged
 * for review — "approved" for export means closed and unflagged, and a payroll
 * file built from a half-finished period is worse than no file.
 */
export async function GET(req: Request) {
    ...
    if (result.blocking.length > 0) {
        return NextResponse.json(
            {
                error: "Some time entries in this period are still open or need review. Clear them on /manager/time-entries first.",
    ...
    return new NextResponse(csv, {
        headers: {
            ...
            // Lets the review page compare a fresh download against the hash
            // stored when the period was locked, without re-reading the file.
            "X-Summary-Hash": result.summaryHash,
        },
    });
}
