...
export default function BudgetClient({ project, data }: { project: { id: string; name: string }; data: BudgetData }) {
    ...
    const [includeStatuses, setIncludeStatuses] = useState(() => {
        const defaults = { estimateSent: true, estimateApproved: true, coApproved: true, coDraft: false };
        if (typeof window === "undefined") return defaults;
        try {
            const stored = localStorage.getItem(`budget-statuses-${project.id}`);
    ...
    function updateIncludeStatuses(updater: (prev: typeof includeStatuses) => typeof includeStatuses) {
        setIncludeStatuses((prev: typeof includeStatuses) => {
            const next = updater(prev);
            try { localStorage.setItem(`budget-statuses-${project.id}`, JSON.stringify(next)); } catch {}
            return next;
        });
    }
    ...
    async function handleCostForecast() {
        ...
        try {
            ...
            const msg = e instanceof Error ? e.message : "Failed to generate forecast";
            toast.error(msg);
        } finally {
    ...
    return (
        <div>
            ...
            <div className="flex items-center justify-between mb-6">
                ...
                <div className="flex items-center gap-2">
                    <button
                        ...
                        className="hui-btn bg-gradient-to-r from-violet-500 to-purple-600 text-white hover:from-violet-600 hover:to-purple-700 disabled:opacity-50 text-sm px-3 py-2 rounded-lg flex items-center gap-1.5 shadow-sm"
                    >
                        {forecastLoading ? (
                        ...
                        {forecastLoading ? "Analyzing..." : "AI Cost Forecast"}
                    </button>
                    <button
            ...
            {forecastResult && (
                <div className="mb-8 bg-gradient-to-br from-violet-50 to-purple-50 border border-violet-200 rounded-xl p-5 relative">
                    <button
                    ...
                    <div className="flex items-center gap-2 mb-3">
                        ...
                        <h3 className="text-sm font-bold text-violet-800 uppercase tracking-wider">AI Cost Forecast</h3>
                    </div>
                    <pre className="whitespace-pre-wrap text-sm text-slate-700 leading-relaxed font-sans">{forecastResult}</pre>
                </div>
            )}
                            ...
                            <div className="space-y-2">
                                <label className="flex items-center gap-2.5 cursor-pointer">
                                    <input
                                        ...
                                        onChange={e => updateIncludeStatuses(s => ({ ...s, estimateSent: e.target.checked }))}
                                ...
                                <label className="flex items-center gap-2.5 cursor-pointer">
                                    <input
                                        ...
                                        onChange={e => updateIncludeStatuses(s => ({ ...s, estimateApproved: e.target.checked }))}
                                        className="rounded border-slate-300 text-hui-primary focus:ring-hui-primary"
                                    />
                                ...
                                <label className="flex items-center gap-2.5 cursor-pointer">
                                    <input
                                        ...
                                        onChange={e => updateIncludeStatuses(s => ({ ...s, coApproved: e.target.checked }))}
                                        className="rounded border-slate-300 text-hui-primary focus:ring-hui-primary"
                                    />
                                ...
                                <label className="flex items-center gap-2.5 cursor-pointer">
                                    <input
                                        ...
                                        onChange={e => updateIncludeStatuses(s => ({ ...s, coDraft: e.target.checked }))}
                                        className="rounded border-slate-300 text-hui-primary focus:ring-hui-primary"
                                    />
