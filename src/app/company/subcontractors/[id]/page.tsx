                ...
                <div className="space-y-6">
                    ...
                    <div className="bg-white rounded-xl shadow-sm border border-hui-border overflow-hidden">
                        <div className="p-4 border-b border-hui-border bg-slate-50/50 flex justify-between items-center">
                            ...
                            <span className={`text-[10px] leading-tight font-bold px-2.5 py-1 rounded-full uppercase tracking-wider ${sub.coiUploaded ? "bg-emerald-100 text-emerald-700" : "bg-red-100 text-red-700"}`}>
                        ...
                        <div className="p-5">
                            ...
                            {sub.coiUploaded ? (
                                <div className="p-3 border border-emerald-200 bg-emerald-50 rounded-lg flex items-center justify-between">
                                    ...
                                    <div className="flex items-center gap-3">
                                        {sub.coiFileUrl && (
                                            <a href={sub.coiFileUrl} target="_blank" rel="noreferrer" className="text-xs font-medium text-emerald-700 hover:text-emerald-900 underline flex items-center gap-1">
                                        ...
                                        <button 
                                            onClick={async () => {
                                                if (!confirm("Remove this Certificate of Insurance?")) return;
                                                try {
                                                    const { deleteSubcontractorCOI } = await import("@/lib/actions");
                                                    await deleteSubcontractorCOI(id);
                                                    setSub(prev => prev ? { ...prev, coiUploaded: false, coiFileUrl: null, coiExpiresAt: null } : prev);
                                                    setForm(f => ({ ...f, coiExpiresAt: "" }));
                                                } catch (e) {
                                                    // handled
                                                }
                                            }}
                                            className="text-xs font-medium text-red-600 hover:text-red-800 underline flex items-center gap-1"
                                        >
