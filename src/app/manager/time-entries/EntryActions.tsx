...
//          re-settles the day (WA meal rules) server-side. Closing an OPEN entry on a
//          logistics job additionally requires job notes — the modal collects them.
...
// browser's zone (src/lib/company-wall-time.ts: DST-gap times are rejected, ambiguous
// fall-back times resolve to the earlier instant). An UNCHANGED field resends the
// row's original instant verbatim — the minute-granular input must not round a
// untouched timestamp's seconds away (Codex gate, PR #437).

import { useEffect, useRef, useState } from "react";
import { useRouter } from "next/navigation";
import { toast } from "sonner";
import { companyWallToInstant, instantToCompanyWall } from "@/lib/company-wall-time";
...
export default function EntryActions({ entryId, userName, startTime, endTime, isLogistics }: Props) {
    ...
    async function saveEdit() {
        ...
        if (start === initial.current.start) {
            ...
            const parsed = companyWallToInstant(start);
            if (!parsed) { toast.error("That start time doesn't exist (check the date — DST)"); return; }
        ...
        if (end) {
            if (end === initial.current.end && endTime) {
                ...
                const parsed = companyWallToInstant(end);
                if (!parsed) { toast.error("That end time doesn't exist (check the date — DST)"); return; }
        ...
        try {
            const res = await fetch(`/api/time-entries/${encodeURIComponent(entryId)}`, {
                ...
                body: JSON.stringify({
                    ...
                    ...(closingOpenEntry && jobNotes.trim() ? { notes: jobNotes.trim() } : {}),
                }),
            });
            ...
            {open && (
                ...
                >
                    ...
                    <label className="block text-sm">
                        ...
                        <input type="datetime-local" autoFocus value={start} onChange={(e) => setStart(e.target.value)} className="hui-input mt-1 w-full text-sm" />
                    </label>
                    <label className="block text-sm">
                    ...
                    {closingOpenEntry && isLogistics && (
                        <label className="block text-sm">
                            <span className="text-xs font-medium text-hui-textMuted">Job notes (required to close a logistics entry)</span>
