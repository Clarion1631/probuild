...
interface Client {
    ...
    taxExemptCertExpiresAt: string | null;
    taxExemptCertNote: string | null;
    projects: { id: string }[];
    leads: { id: string }[];
}
                ...
                <table className="w-full text-sm">
                    <thead>
                        <tr className="text-left text-xs text-hui-textMuted uppercase tracking-wide border-b border-hui-border bg-hui-surface">
                            ...
                            <th className="px-4 py-3 text-center">Tax Cert</th>
                    ...
                    <tbody>
                        {filtered.length === 0 && (
                            <tr>
                                <td colSpan={8} className="px-4 py-10 text-center text-hui-textMuted text-sm">
                        ...
                        {filtered.map(c => (
                            <Fragment key={c.id}>
                            <tr className="border-b border-hui-border last:border-0 hover:bg-hui-surface/50">
                                {editId === c.id ? (
                                    <>
                                    ...
                                    <>
                                        ...
                                        <td className="px-4 py-3 text-center">
                                            {c.taxExemptCertUrl ? (
                                                getTaxCertStatus({ url: c.taxExemptCertUrl, expiresAt: c.taxExemptCertExpiresAt }) === "expired" ? (
                                                    <span className="text-xs font-semibold px-2 py-0.5 rounded-full bg-red-100 text-red-700">Expired</span>
                                                ) : (
                            ...
                            {editId === c.id && (
                                <tr className="border-b border-hui-border bg-hui-surface/50">
                                    <td colSpan={8} className="px-4 py-3">
                                        <div className="max-w-md">
                                            <TaxExemptCertCard
                                                clientId={c.id}
                                                certUrl={c.taxExemptCertUrl}
                                                certExpiresAt={c.taxExemptCertExpiresAt}
                                                certNote={c.taxExemptCertNote}
                                                onSaved={cert => setClients(prev => prev.map(cl => cl.id === c.id ? { ...cl, ...cert } : cl))}
                                            />
                                        </div>
                                    </td>
                                </tr>
