...
test("the retry pass re-posts unposted rows, and selects only when it may", () => {
    ...
    assert.match(source, /const selectionAllowed = chaserCompletedFor\(marker, date, "America\/Los_Angeles", currentCycleId\);/);
...
test("the sweep is time-budgeted, checkpoints per batch, and stops at a failure", () => {
    ...
    assert.match(source, /await writeCursor\(formatSweepCursor\(\{ key: cursor, epoch: snapshotEpoch, evidenceEpoch: snapshotEvidenceEpoch \}\)\)/);
