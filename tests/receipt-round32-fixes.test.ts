...
test("the freshness window is the one the cron schedule implies", () => {
    ...
    // Every five minutes, but OFFSET off the hour so it never collides with the
    // 13:00 full run (round-45 gate, finding 2).
    assert.match(resume as string, /^2-59\/5 /, "every five minutes, offset, so it runs many times before 14:30");
});
...
test("the sweep reads the pull marker the pull actually writes", () => {
    ...
    assert.match(pull, /const stampWarranted = summary\.ok && summary\.complete && summary\.clearedProbeOk && ambiguousCount === 0[\s\S]{0,140}?quarantineHeld\.length === 0 && !quarantineBlocked[\s]*&& !summary\.uncertified;/);
