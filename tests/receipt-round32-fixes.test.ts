...
test("the block is reported where a human will see it, not only in the summary", () => {
    ...
    // A fourth since cheap-sweep-restart §14.6 (Codex round 2 blocker 2): an
    // eligible line the walk left undecided with no open issue covering it.
    assert.match(sweep, /\.\.\.\(bankPullStale[\s]*\? \{ reason: BANK_PULL_STALE_REASON \}[\s]*: ledgerMoved \? \{ reason: PULL_MOVED_REASON \}[\s]*: fenceFailed \? \{ reason: LEDGER_FENCE_FAILED_REASON \}[\s]*: undecidedBlocking \? \{ reason: UNDECIDED_LINES_REASON \} : \{\}\)/);
...
test("the sweep reads the pull marker the pull actually writes", () => {
    ...
    assert.match(pull, /const stampWarranted = summary\.ok && summary\.complete && summary\.clearedProbeOk && ambiguousCount === 0\s*&& quarantineHeld\.length === 0 && !quarantineBlocked(?:\s|\/\/[^\n]*|\/\*[\s\S]*?\*\/)*&& conflictOutcome\.ok(?:\s|\/\/[^\n]*|\/\*[\s\S]*?\*\/)*&& !summary\.conflictFlood(?:\s|\/\/[^\n]*|\/\*[\s\S]*?\*\/)*&& !summary\.uncertified;/);
