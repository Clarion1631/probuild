...
test("the sweep reads the pull marker the pull actually writes", () => {
    ...
    assert.match(pull, /const stampWarranted = summary\.ok && summary\.complete && summary\.clearedProbeOk && ambiguousCount === 0[\s\S]{0,140}?quarantineHeld\.length === 0 && !quarantineBlocked[\s\S]{0,1200}?&& conflictOutcome\.ok[\s]*&& !summary\.uncertified;/);
