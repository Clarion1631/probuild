...
const CLIENT_HANDLERS: Array<{ handler: string; action: string; closeCall: string }> = [
    { handler: "handleSplit", action: "splitInvoiceMilestones", closeCall: "setShowSplit(false)" },
    { handler: "handleSaveEdit", action: "updatePendingMilestoneAmounts", closeCall: "handleCancelEditMode()" },
    { handler: "handleDeleteMilestone", action: "deleteInvoiceMilestone", closeCall: "setDeleteMilestoneTarget(null)" },
];

for (const { handler, action, closeCall } of CLIENT_HANDLERS) {
    test(`${handler}: bails out on { success: false } before touching UI state`, () => {
        ...
        // 3. the guard body reports the reason and bails out.
