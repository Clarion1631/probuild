...
test("the bank pull records its last COMPLETE success, not its last run", () => {
    ...
    assert.match(source, /const stampWarranted = summary\.ok && summary\.complete && summary\.clearedProbeOk && ambiguousCount === 0\s*&& quarantineHeld\.length === 0 && !quarantineBlocked(?:\s|\/\/[^\n]*|\/\*[\s\S]*?\*\/)*&& conflictOutcome\.ok(?:\s|\/\/[^\n]*|\/\*[\s\S]*?\*\/)*&& !summary\.conflictFlood(?:\s|\/\/[^\n]*|\/\*[\s\S]*?\*\/)*&& !summary\.uncertified;/);
