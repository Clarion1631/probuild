...
test("the bank pull records its last COMPLETE success, not its last run", () => {
    ...
    assert.match(source, /const stampWarranted = summary\.ok && summary\.complete && summary\.clearedProbeOk && ambiguousCount === 0[\s\S]{0,140}?quarantineHeld\.length === 0 && !quarantineBlocked[\s\S]{0,1200}?&& conflictOutcome\.ok[\s]*&& !summary\.uncertified;/);
